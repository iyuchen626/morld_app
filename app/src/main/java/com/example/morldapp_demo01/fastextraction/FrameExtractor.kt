package com.ducky.fastvideoframeextraction.fastextraction
...
class FrameExtractor(private val listener: IVideoFrameExtractor) {
    ...
    fun extractFrames(inputFilePath: String) {
        ...
        try {
            ...
            val mediaMetadataRetriever = MediaMetadataRetriever()
            mediaMetadataRetriever.setDataSource(inputFile.absolutePath)
            val orientation: Int = mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_ROTATION)?.toInt() ?: 0
            val count : Int = mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_FRAME_COUNT)?.toInt() ?: 1
            Log.d(TAG, "Orientation: $orientation")

            // Checking duration by milliseconds
            val duration : Int = mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)?.toInt() ?: 1
                ...
                if (inputBufIndex >= 0) {
                    ...
                    if (chunkSize < 0) {
                        ...
                        if (verbose) {
                            Log.d(
                                TAG,
                                ("submitted frame $inputChunk to dec, size=$chunkSize at $presentationTimeUs")
                            )
                        }
                        inputChunk++
                        extractor.advance()
                    ...
                    if (doRender) {
                        ...
                        if (Math.abs(lastHandlerMicrosecond - presentationTimeUs) >= ((1.0/(FPS*0.25))) * 1000 * 1000) {
                            ...
                            if (decodeCount < MAX_FRAMES) {
                                ...
                                val currentFrame =
                                    outputSurface.retrieveFrame(decodeCount, presentationTimeUs)
