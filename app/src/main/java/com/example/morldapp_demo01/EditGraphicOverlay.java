...
/**
 * A view which renders a series of custom Editgraphics to be overlayed on top of an associated preview
 * (i.e., the camera preview). The creator can add Editgraphics objects, update the objects, and remove
 * them, triggering the appropriate drawing and invalidation within the view.
 *
 * <p>Supports scaling and mirroring of the Editgraphics relative the camera's preview properties. The
 ...
 *
 * <p>Associated {@link EditGraphic} items should use the following methods to convert to view
 * coordinates for the Editgraphics that are drawn:
 *
 * <ol>
 *   <li>{@link EditGraphic#scale(float)} adjusts the size of the supplied value from the image scale to
 *       the view scale.
 *   <li>{@link EditGraphic#translateX(float)} and {@link EditGraphic#translateY(float)} adjust the
 *       coordinate from the image's coordinate system to the view coordinate system.
 * </ol>
 */
public class EditGraphicOverlay extends View {
  private final Object lock = new Object();
  private final List<EditGraphic> graphics = new ArrayList<>();
  ...
  /**
   * Base class for a custom Editgraphics object to be rendered within the gEditraphic overlay. Subclass
   * this and implement the {@link EditGraphic#draw(Canvas)} method to define the Editgraphics element. Add
   * instances to the overlay using {@link EditGraphicOverlay#add(EditGraphic)}.
   */
  public abstract static class EditGraphic {
    private EditGraphicOverlay overlay;

    public EditGraphic(EditGraphicOverlay overlay) {
      this.overlay = overlay;
    }

    /**
     * Draw the Editgraphic on the supplied canvas. Drawing should use the following methods to convert
     * to view coordinates for the Editgraphics that are drawn:
     *
     * <ol>
     *   <li>{@link EditGraphic#scale(float)} adjusts the size of the supplied value from the image
  ...
  public EditGraphicOverlay(Context context, AttributeSet attrs) {
    super(context, attrs);
  ...
  public void add(EditGraphic graphic) {
    synchronized (lock) {
  ...
  public void remove(EditGraphic graphic) {
    synchronized (lock) {
  ...
  /** Draws the overlay with its associated EditGraphic objects. */
  @Override
  protected void onDraw(Canvas canvas) {
    ...
    synchronized (lock) {
      ...
      for (EditGraphic graphic : graphics) {
        graphic.draw(canvas);
